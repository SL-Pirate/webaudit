...
class PlaywrightWebAuditServiceTest {
    ...
    class Audit {
        ...
        void orchestratesFullAudit() {
            String url = serveSafe("/audit/full", """
                    <html>
                    <head>
                        <title>Audit Page</title>
                        <meta name="description" content="Test audit.">
                    </head>
                    <body>
                        <h1>Hello Audit</h1>
                        <p>Some content here for the audit to process.</p>
            ...
            // Stub AI service
            var fakeInsights = new WebAuditResult.AiInsights(
                    "Good structure", "Clear messaging", "CTA is fine",
                    "Sufficient depth", "No UX concerns"
            );
            var fakeRecs = List.of(
                    new WebAuditResult.Recommendation(1, "Add more headings", "Only one h1")
            );
            when(aiService.generateInsights(any(), anyString())).thenReturn(fakeInsights);
            when(aiService.recommendations(any(), any(), anyString())).thenReturn(fakeRecs);

            WebAuditResult result = service.audit(url);

            assertNotNull(result);
            assertNotNull(result.factualMetrics());
            assertEquals(fakeInsights, result.aiInsights());
            assertEquals(fakeRecs, result.recommendations());
            assertEquals("Audit Page", result.factualMetrics().metaTitle());
        }

        @Test
        void auditCallsReadContentTwice_potentialInefficiency() {
            // The audit() method calls stripHtml() and getFactualMetrics() which each
            // navigate to the URL separately. This test documents that behaviour works
            // correctly (even if it's two separate navigations).
            String url = serveSafe("/audit/twice", """
                    <html>
                    <head><title>Double Nav</title></head>
                    <body><p>Content</p></body>
                    </html>
                    """);

            var fakeInsights = new WebAuditResult.AiInsights("a", "b", "c", "d", "e");
            when(aiService.generateInsights(any(), anyString())).thenReturn(fakeInsights);
            ...
            assertNotNull(result);
            assertEquals("Double Nav", result.factualMetrics().metaTitle());
        }
    }
